...
func TestRawHTMLRemainsASTNode(t *testing.T) {
	doc, err := New(WithHostPolicy(Policy{RawHTML: RawHTMLSanitized, OutputBytes: MaxOutputBytes})).Compile(context.Background(), Source{Name: "raw.md", Content: []byte("<span>raw</span>")})
	if err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
