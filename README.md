...
links and fragments, and writes `margo-manifest.json`, `sitemap.xml`, and
`llms.txt` for configured sites. Output is staged beside the destination and
published by rename only after a successful build.

Pages can opt into source and download controls with frontmatter:

```yaml
margo:
  actions:
    markdown: true
    pdf: true # pre-rendered PDF + direct download
```

For client-side rendering instead:

```yaml
margo:
  actions:
    pdf: client # browser render + Print / Save PDF
```

`markdown: true` retains the original `.md` beside the generated `.html` and
adds a compact page-actions dropdown with Copy page and View as Markdown. Set
`pdf: true` to also generate a `.pdf`; PDF publication implies Markdown
retention and adds Download PDF. Use `pdf: client` to skip PDF generation and
use the browser's current page for Print / Save PDF instead. Pre-rendered PDFs
remain the default in both light and dark site themes; client printing follows
...
Pre-rendered PDFs include the materialized document brand in the PDF HTML
projection: a header with the configured logo and site name, plus a footer with
the document title and `Generated by Margo`. This furniture is engine-neutral;
it does not depend on the surrounding site shell.
