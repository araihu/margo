...
func TestStandaloneBrandedFooterTravelsWithLastContentBlock(t *testing.T) {
	result := mustRenderSource(t, "# Pagination\n\nBody content.\n")
	component, err := RenderStandalone(result, WithBrand(Brand{Footer: templ.Raw(`<span>Generated by Margo</span>`)}))
	if err != nil {
		t.Fatal(err)
	}
	markup := renderComponent(t, component)
	endmatterIndex := strings.Index(markup, `class="goshtoso-document__endmatter"`)
	lastContentIndex := strings.Index(markup, `Body content.</p>`)
	footerIndex := strings.Index(markup, `class="goshtoso-document__footer"`)
	if endmatterIndex < 0 || lastContentIndex < endmatterIndex || footerIndex < 0 || footerIndex < lastContentIndex {
		t.Fatalf("branded footer is not attached after the last content block: %s", markup)
	}
	if strings.Count(markup, `class="goshtoso-document__footer"`) != 1 {
		t.Fatalf("branded footer count = %d; want 1", strings.Count(markup, `class="goshtoso-document__footer"`))
	}
}
